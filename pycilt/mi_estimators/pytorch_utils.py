...
def get_mine_loss(preds_xy, preds_xy_tilde, metric):
    ...
    if metric == 'donsker_varadhan':
        return preds_xy.mean(dim=0) - log_mean_exp(preds_xy_tilde, dim=0)
    elif metric == 'donsker_varadhan_softplus':
        return torch.log(F.softplus(preds_xy) + SMALL).mean(dim=0) - torch.log(
            F.softplus(preds_xy_tilde).mean(dim=0) + SMALL)
    elif metric == 'fdivergence':
        return preds_xy.mean(dim=0) - torch.exp(preds_xy_tilde - 1).mean(dim=0)
    else:
